...
# Optional desktop hardware support.
# LOVESICK still runs if these packages/devices are unavailable.
try:
    import cv2
except Exception:
    cv2 = None
...
class MicLevelBar(Widget):
    ...
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        with self.canvas.before:
            self.track_color = Color(0.10, 0.07, 0.12, 0.96)
            self.track = RoundedRectangle(pos=self.pos, size=self.size, radius=[9])
            self.fill_color = Color(0.35, 0.88, 0.66, 1)
            self.fill = RoundedRectangle(pos=self.pos, size=(0, self.height), radius=[9])
        self.bind(pos=self._sync, size=self._sync, level=self._sync)

    def _sync(self, *_):
        self.track.pos = self.pos
        self.track.size = self.size
        amount = max(0.0, min(1.0, float(self.level)))
        self.fill.pos = self.pos
        self.fill.size = (self.width * amount, self.height)

        # Visual feedback only: green -> yellow -> red as input gets louder.
        if amount < 0.55:
            self.fill_color.rgba = (0.35, 0.88, 0.66, 1)
        elif amount < 0.82:
            self.fill_color.rgba = (1.00, 0.82, 0.28, 1)
        else:
            self.fill_color.rgba = (1.00, 0.34, 0.38, 1)
...
STORY = {
    ...
    "date": {
        ...
        "choices": [("Agree to meet.", "date_bridge_1", 3, 0), ("I'm not going with you.", "refuse_bridge_1", -3, 0)]
    },
...
class GameScreen(Screen):
    ...
    def choose(self, next_node, yande_change, ace_change):
        ...
        elif self.current_node == "date" and next_node == "date_bridge_1":
            game.route_choice = "agree"

        if next_node == "date_game":
            self.manager.current = "date_game"
            return
...
class YandeDateScreen(Screen):
    """Yan'De's 15-question date game.

    Desktop test mode uses the real webcam and microphone when the player
    explicitly presses ALLOW. Nothing is written to disk. Device streams are
    closed when the sequence finishes or the screen is left.
    """
    ...
    ROMANCE_QUESTIONS = [
        ...
        ("If we had an entire day together, would you want to go somewhere or stay home?",
         "Go somewhere", "Stay together"),
    ...
    def build_ui(self):
        ...
        # Dedicated webcam window. It stays hidden until camera permission is granted.
        self.camera_panel = Panel(
            bg_color=(0.025, 0.018, 0.032, 0.98),
            radius=18,
        ...
        self.camera_title.bind(size=lambda obj, value: setattr(obj, "text_size", value))
        self.preview = Image(fit_mode="contain")
        self.camera_panel.add_widget(self.camera_title)
        self.camera_panel.add_widget(self.preview)
        self.root.add_widget(self.camera_panel)

        # Live microphone level meter. It appears only while LOVESICK has the mic open.
        self.mic_panel = Panel(
            bg_color=(0.025, 0.018, 0.032, 0.98),
            radius=16,
            orientation="vertical",
            padding=(10, 7),
        ...
        self.mic_label = Label(
            ...
            font_size=13,
            bold=True,
            color=(0.78, 0.95, 0.86, 1),
            size_hint_y=None,
            height=24,
            halign="left",
            valign="middle",
        )
        self.mic_label.bind(size=lambda obj, value: setattr(obj, "text_size", value))
        self.mic_bar = MicLevelBar(level=0.0, size_hint_y=None, height=18)
        self.mic_panel.add_widget(self.mic_label)
        self.mic_panel.add_widget(self.mic_bar)
        self.root.add_widget(self.mic_panel)

        self.yande_sprite = Image(
            source=SPRITES["Yan'De"]["playful"],
            fit_mode="contain",
            size_hint=(0.62, 0.78),
        ...
        self.panel = Panel(
            bg_color=(0.055, 0.035, 0.070, 0.97),
            radius=22,
            orientation="vertical",
            padding=(18, 12),
            spacing=7,
        ...
        self.counter.bind(size=lambda obj, value: setattr(obj, "text_size", value))
        ...
        self.dialogue.bind(size=lambda obj, value: setattr(obj, "text_size", value))
        ...
        self.panel.add_widget(self.counter)
        self.panel.add_widget(self.dialogue)
        self.panel.add_widget(self.choice_box)
        self.root.add_widget(self.panel)
        self.add_widget(self.root)

    def set_expression(self, expression):
        self.yande_sprite.source = SPRITES["Yan'De"].get(
            expression, SPRITES["Yan'De"]["neutral"]
        )
    ...
    def show_round(self):
        self.waiting = False
        self.choice_box.clear_widgets()
        self.counter.text = f"YAN'DE'S QUESTION GAME   {self.round_number} / 15"

        if self.round_number <= 10:
            question, a, b = self.ROMANCE_QUESTIONS[self.round_number - 1]
            self.dialogue.text = f"YAN'DE: \"{question}\""
            self.set_expression("playful" if self.round_number < 8 else "soft")
            self.add_choice(a, lambda *_: self.answer_button(a))
            self.add_choice(b, lambda *_: self.answer_button(b))
            self.status.text = ""
            return

        if self.round_number == 11 and not self.mic_allowed:
            self.set_expression("soft")
            self.dialogue.text = (
                "YAN'DE: \"These buttons are getting boring.\"\n"
                "\"Talk to me instead.\""
            )
            self.show_permission_prompt("microphone")
            return

        if self.round_number == 14 and not self.camera_allowed:
            self.set_expression("serious")
            self.dialogue.text = (
                "YAN'DE: \"You know what's bothering me?\"\n"
                "\"I can hear you... but I still can't see you.\"\n"
                "\"Let's fix that.\""
            )
            self.show_permission_prompt("camera")
            return

        self.begin_voice_round()

    def add_choice(self, label, callback):
        btn = Button(text=label, font_size=16, size_hint_y=None, height=56)
        btn.bind(on_release=callback)
        self.choice_box.add_widget(btn)

    def answer_button(self, answer):
        if self.waiting:
            return
        self.waiting = True
        self.choice_box.clear_widgets()
        self.dialogue.text = f"AZZY: \"{answer}\"\n\nYAN'DE: \"Cute.\""
        Clock.schedule_once(self.advance_round, 0.8)

    def show_permission_prompt(self, device):
        self.choice_box.clear_widgets()
        pretty = "MICROPHONE" if device == "microphone" else "CAMERA"
        self.status.text = f"DESKTOP TEST MODE • {pretty} ACCESS REQUEST"
        self.add_choice(
            f"ALLOW {pretty}",
            lambda *_: self.permission_response(device, True)
        )
        self.add_choice(
            f"DENY {pretty}",
            lambda *_: self.permission_response(device, False)
        )

    def permission_response(self, device, allowed):
        self.choice_box.clear_widgets()
        ...
        if device == "microphone":
            if allowed:
                if self.start_microphone():
                    ...
                    self.status.text = "🎙 MICROPHONE ACTIVE • LOCAL TEST MODE"
                    self.dialogue.text = "YAN'DE: \"There. That's better.\"\n\"Now answer me out loud.\""
                else:
                    self.mic_allowed = False
                    self.status.text = "MICROPHONE UNAVAILABLE"
                    self.dialogue.text = (
                        "YAN'DE: \"...Your microphone won't open.\"\n"
                        "\"Fine. I'll pretend I heard you.\""
                    )
            else:
                self.mic_allowed = False
                self.status.text = "MICROPHONE DENIED"
                self.dialogue.text = (
                    "YAN'DE: \"You won't talk to me?\"\n"
                    "\"You're making this difficult.\""
                )
            Clock.schedule_once(lambda dt: self.begin_voice_round(), 1.0)
            return
        ...
        if device == "camera":
            if allowed:
                if self.start_camera():
                    ...
                    self.status.text = self.device_status_text()
                    self.dialogue.text = "YAN'DE: \"...\"\n\"There you are.\""
                    self.set_expression("obsessed")
                else:
                    self.camera_allowed = False
                    self.status.text = "CAMERA UNAVAILABLE"
                    self.dialogue.text = (
                        "YAN'DE: \"Your camera won't open.\"\n"
                        "\"Still hiding from me, huh?\""
                    )
            else:
                self.camera_allowed = False
                self.status.text = "CAMERA DENIED"
                self.dialogue.text = (
                    "YAN'DE: \"Oh.\"\n"
                    "\"So you are hiding from me.\""
                )
            Clock.schedule_once(lambda dt: self.begin_voice_round(), 1.2)
    ...
    def start_microphone(self):
        ...
        try:
            ...
            self.mic_panel.opacity = 1
            self.mic_panel.disabled = False
            if self.mic_meter_event is not None:
                self.mic_meter_event.cancel()
            self.mic_meter_event = Clock.schedule_interval(self.update_mic_meter, 1 / 30)
            return True
        except Exception as exc:
            print("LOVESICK microphone test mode:", exc)
            self.mic_stream = None
            return False

    def update_mic_meter(self, _dt):
        # Map RMS input to a readable 0-100% display.
        # The multiplier intentionally makes ordinary speech visible without
        # changing or amplifying the actual audio stream.
        normalized = max(0.0, min(1.0, float(self.mic_level) * 14.0))
        # Smooth the visual bar so it does not flicker harshly.
        current = float(self.mic_bar.level)
        smoothed = (current * 0.68) + (normalized * 0.32)
        self.mic_bar.level = smoothed
        self.mic_label.text = f"MIC INPUT   {int(round(smoothed * 100))}%"
        return True

    def stop_microphone(self):
        stream = getattr(self, "mic_stream", None)
        ...
        event = getattr(self, "mic_meter_event", None)
        if event is not None:
            event.cancel()
        self.mic_meter_event = None

        if hasattr(self, "mic_bar"):
            self.mic_bar.level = 0
        if hasattr(self, "mic_label"):
            self.mic_label.text = "MIC INPUT   0%"
        if hasattr(self, "mic_panel"):
            self.mic_panel.opacity = 0
            self.mic_panel.disabled = True
    ...
    def start_camera(self):
        ...
        try:
            ...
            self.camera_panel.opacity = 1
            self.camera_panel.disabled = False
            self.camera_event = Clock.schedule_interval(self.update_camera, 1 / 24)
            return True
        except Exception as exc:
            print("LOVESICK camera test mode:", exc)
            self.camera_capture = None
            return False
    ...
    def update_camera(self, _dt):
        ...
        try:
            # Analyze only the live frame in memory; nothing is saved.
            mean_bgr = frame.mean(axis=(0, 1))
            brightness = float(mean_bgr.mean())
            self.last_frame_stats = (brightness, mean_bgr.tolist())
    ...
    def stop_camera(self):
        event = getattr(self, "camera_event", None)
        if event is not None:
            event.cancel()
        self.camera_event = None
        ...
        if hasattr(self, "camera_panel"):
            self.camera_panel.opacity = 0
            self.camera_panel.disabled = True

    def device_status_text(self):
        bits = []
        if self.mic_allowed:
            bits.append("🎙 MIC ACTIVE")
        if self.camera_allowed:
            bits.append("📷 CAMERA ACTIVE")
        return " • ".join(bits) if bits else ""

    def begin_voice_round(self):
        if self.round_number < 11:
            return

        self.choice_box.clear_widgets()
        prompt = self.VOICE_PROMPTS[self.round_number]
        self.voice_detected = False

        if self.mic_allowed:
            self.status.text = self.device_status_text() + " • LISTENING..."
            self.dialogue.text = f"YAN'DE: \"{prompt}\"\n\n[Speak your answer out loud.]"
            self.add_choice("DONE SPEAKING", self.finish_voice_round)
        else:
            self.status.text = ("📷 CAMERA ACTIVE" if self.camera_allowed else "") + " • MIC OFFLINE"
            self.dialogue.text = (
                f"YAN'DE: \"{prompt}\"\n\n"
                "[Microphone unavailable/denied. Continue when ready.]"
            )
            self.add_choice("CONTINUE", self.finish_voice_round)

    def camera_observation(self):
        stats = self.last_frame_stats
        if not stats:
            return "You're really there."
        brightness, bgr = stats
        b, g, r = bgr

        if brightness < 65:
            return "It's pretty dark around you."
        if brightness > 185:
            return "You're somewhere bright."

        dominant = max((b, "blue"), (g, "green"), (r, "red"))[1]
        spread = max(b, g, r) - min(b, g, r)
        if spread > 28:
            return f"There's a lot of {dominant} in your camera."
        return "I can see you clearly enough."

    def finish_voice_round(self, *_):
        if self.waiting:
            return
        self.waiting = True
        self.choice_box.clear_widgets()

        heard = self.voice_detected
        n = self.round_number
        ...
        if n == 11:
            reply = (
                ...
                if heard else
                "\"You're quiet. That's okay. I can wait.\""
            )
        elif n == 12:
            reply = (
                "\"Mm. I'll remember that.\""
                if heard else
                "\"Keeping your perfect relationship a secret from me?\""
            )
        elif n == 13:
            self.set_expression("obsessed")
            reply = (
                ...
                if heard else
                "\"You couldn't even say my name? That's mean.\""
            )
        elif n == 14:
            self.set_expression("obsessed")
            observation = self.camera_observation() if self.camera_allowed else "Still hiding from me."
        ...
        self.dialogue.text = f"YAN'DE: {reply}"
        self.status.text = self.device_status_text()

        if n == 15:
            Clock.schedule_once(self.cut_the_feed, 1.3)
        else:
            Clock.schedule_once(self.advance_round, 1.0)

    def advance_round(self, _dt):
        self.round_number += 1
        self.show_round()

    def cut_the_feed(self, _dt):
        # Yan'De appears to "mute" the player, but technically LOVESICK simply
        # closes the streams that the player explicitly allowed this app to use.
        self.stop_microphone()
        self.stop_camera()
        self.status.text = "MICROPHONE OFFLINE   •   CAMERA OFFLINE"
        self.dialogue.text = (
        ...
        self.choice_box.clear_widgets()
        self.add_choice("END DATE", self.finish_date)

    def finish_date(self, *_):
        self.stop_microphone()
        self.stop_camera()
        game.date_completed = True
        game.date_result = "completed"

        # For now the date returns to the main story's normal ending logic.
        # This keeps the minigame playable while its final route outcomes are
        # designed separately.
        game_screen = self.manager.get_screen("game")
        game_screen.resume_node = "confront"
        self.manager.current = "game"
...
class LovesickApp(App):
    def build(self):
        ...
        manager.add_widget(YandeDateScreen(name="date_game"))
