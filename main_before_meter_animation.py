...
STORY = {
    ...
    "date": {
        ...
        "choices": [("Agree to meet.", "doctor", 3, 0), ("Refuse.", "doctor", -3, 0)]
    },
    "doctor": {
        "speaker": "Dr. Ace Brighton", "expression": "neutral",
        "text": "Ace closes Azzy's file.\n\n\"I'd like to speak with Yan'De directly. One session. No audience.\"\n\nFor the next part, you will play as Dr. Brighton.",
        "choices": [("Begin Therapy Session", "therapy", 0, 0)]
    },
    "after_therapy": {
        "speaker": "Dr. Ace Brighton", "expression": "smiling",
        "text": "The office finally goes quiet.\n\nAce exhales. \"That could have gone worse.\"",
        "choices": [("Continue", "confront", 0, 3)]
    },
    "confront": {
...
class TherapyScreen(Screen):
    ...
    def build_session_ui(self):
        ...
        self.insanity_bar = ProgressBar(max=100, value=game.yande_insanity)
    ...
    def update_insanity_ui(self):
        ...
        self.insanity_bar.value = value

        if value >= 75 and value < 100:
            Animation.cancel_all(self.insanity_panel)
            Animation(opacity=0.65, duration=0.18) + Animation(opacity=1.0, duration=0.18)
    ...
    def finish_session(self):
        ...
        # Reward a calmer session with a stronger Ace relationship.
        if game.yande_insanity < 50:
            game.ace_relationship += 4
        elif game.yande_insanity < 75:
            game.ace_relationship += 2
        else:
            game.ace_relationship += 1
        game.ace_relationship = min(100, game.ace_relationship)

        game_screen = self.manager.get_screen("game")
        game_screen.resume_node = "after_therapy"
        self.manager.current = "game"
